...
_IMX_UBOOT_COPY_FILES = $(UBOOT_OUT_SPL_BIN) $(UBOOT_OUT_BIN) $(UBOOT_OUT_NODTB_BIN) $(UBOOT_DTB_FULL)
_IMX_UBOOT_FILENAMES=$(notdir $(_IMX_UBOOT_COPY_FILES))
_IMX_MKIMAGE_TARGET := $(IMX_MKIMAGE_FIRMWARE_DEST)/.mkimage-files-copied
_IMX_MKIMAGE_IMX8 = $(IMX_MKIMAGE_DEST)/mkimage_imx8

.PHONY: imx_mkimage
imx_mkimage: $(IMX_MKIMAGE_OUT_FLASH_BIN)
$(IMX_MKIMAGE_OUT_FLASH_BIN): $(_IMX_MKIMAGE_IMX8) $(_IMX_MKIMAGE_TARGET) | $(IMX_MKIMAGE_DEST)/.git
	# note: we need to cd... because the scripts use $(PWD)
	cd $(IMX_MKIMAGE_DEST); make $(IMX_MKIMAGE_FLAGS) $(IMX_MKIMAGE_MK_TARGET)

$(_IMX_MKIMAGE_IMX8): $(IMX_MKIMAGE_DEST)/.git
	cd $(IMX_MKIMAGE_DEST); make CFLAGS="$(IMX_MKIMAGE_CFLAGS)" bin
...
.PHONY: uuu_spl
uuu_spl: $(UUU)
	$(UUU_CMD) -b spl $(IMX_OUT_FLASH_BIN)
